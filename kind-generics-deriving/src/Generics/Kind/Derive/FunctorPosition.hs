...
instance (Interpret c as => GFunctorPos f v a as b bs, {- Interpret c as => -} Interpret c bs)
         => GFunctorPos (c :=>: f) v a as b bs where
  gfmapp p v (SuchThat x) = SuchThat (gfmapp p v x)

instance forall k f v a as b bs.
         (forall (t :: k). GFunctorPos f ('VS v) a (t ':&&: as) b (t ':&&: bs))
         => GFunctorPos (Exists k f) v a as b bs where
...
class GFunctorArgPos (t :: Atom d (*)) (v :: TyVar d *)
  ...
  gfmappf :: Proxy t -> Proxy v -> Proxy as -> Proxy bs
          -> (a -> b) -> Interpret t as -> Interpret t bs

instance forall t v a as b bs. GFunctorArgPos t v a as b bs
         => GFunctorPos (Field t) v a as b bs where
...
instance forall f y x v a as b bs.
         (Functor (f (Interpret y as)), Interpret y as ~ Interpret y bs, GFunctorArgPos x v a as b bs)
